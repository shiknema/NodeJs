...
const API_URL = 'https://en.wikipedia.org/w/api.php?action=parse&format=json&section=0&page=sweden';
...
function setResponse(responseData) {
    return `<h2>${responseData.country} Information</h2> <p>${JSON.stringify(responseData.data)}</p>`;
};
...
async function getCountryInfo(req, res, next) {
    try {
        ...
        if (data.parse) {
            responseData = { country: data.parse.title, data: JSON.stringify(data.parse)};
        } else {
